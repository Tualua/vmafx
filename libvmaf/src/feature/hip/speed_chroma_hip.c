...
static int run_score_sc(SpeedChromaHipState *s, float *score_out)
{
    ...
    /* H2D eigenvalues (computed in run_cpu_linalg_sc). */
    hipError_t rc = hipMemcpyAsync(s->d_eigenvalues, s->h_eigenvalues, SC_ELEMENTS * sizeof(float),
                                   hipMemcpyHostToDevice, s->stream);
    if (rc != hipSuccess)
        return hip_rc(rc);
                              ...
                              VmafFeatureCollector *feature_collector)
{
    ...
    for (int ch = 1; ch <= 2; ++ch) {
        void *d_indterm = (ch == 1) ? s->d_indterm_ref : s->d_indterm_dis;
        void *d_sol = (ch == 1) ? s->d_sol_ref : s->d_sol_dis;
        ...
        e = run_gpu_pipeline_sc(s, s->h_plane_dis, d_indterm, h_indterm);
        if (!e) {
            memcpy(s->h_cov_mat, saved_cov, sizeof(saved_cov));
            e = run_cpu_linalg_sc(s, h_indterm, d_sol);
        }
