...
#define VIF_BUF_CNT 10
    ...
    for (unsigned scale = scale_start; scale < 4; ++scale) {
        ...
        char pathbuf[256];
#endif
        if (precomputed_filters && precomputed_filter_widths) {
            /* ADR-0500 Win #3: use pre-computed Gaussian filters from VifState.init().
             * Eliminates 4 × vif_get_filter() transcendental calls per frame. */
            filter_width = precomputed_filter_widths[scale];
            memcpy(filter, precomputed_filters[scale], (size_t)filter_width * sizeof(filter[0]));
        } else {
