...
case "$backend" in
  ...
  all)
    command -v nvcc >/dev/null || {
      echo "nvcc missing for --backend=all" >&2
      exit 3
    }
    command -v icpx >/dev/null || {
      echo "icpx missing for --backend=all" >&2
      exit 3
    }
...
if [[ "$san" != none ]]; then
  if [[ "$san" == *tsan* ]]; then
    if [[ "$san" == *asan* || "$san" == *ubsan* ]]; then
      echo "tsan is mutually exclusive with asan/ubsan" >&2
      exit 2
    fi
